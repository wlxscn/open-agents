...
- Open Agents uses `better-auth`'s catch-all auth handler, so Vercel OAuth callbacks resolve to `/api/auth/callback/vercel`, not `/api/auth/vercel/callback`; deployment docs and OAuth app setup must use the `callback/<provider>` form.
- Auth setup reads `BETTER_AUTH_SECRET` from `apps/web/lib/auth/config.ts`; older `JWE_SECRET` docs are stale and will break deployments if copied verbatim.
- Vercel OAuth sign-in can create users without a `username` field populated; synthesize a fallback in `databaseHooks.user.create.before` before inserting into the `users` table, because this schema requires `username NOT NULL`.
- `bun run web` runs the Next app from `apps/web`, so `apps/web/.env` is the active local env file; a root-level `.env.local` does not override it unless the app itself loads that file.
